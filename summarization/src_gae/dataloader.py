...
def get_graph_data(src, clss, pre_src, pre_graphs, pre_clss, pre_edge_importance, batch_size, device):
    ...
    for bi in range(batch_size):
        ...
        for i, e in enumerate(pre_graphs[bi]):
            if e[0] < n_node and e[1] < n_node:
                ...
                graph_importance.append(map_imp(pre_edge_importance[bi][i]))
                graph_importance.append(map_imp(pre_edge_importance[bi][i]))
        graphs.append(graph)
        graphs_importance.append(graph_importance)
    ...
    data_list = [pyg.data.Data(num_nodes=len(src[0])+len(clss[0]), \
            importance=torch.tensor(graphs_importance[i]), \
...
def gae_collate_fn(data):
    ...
    if data is not None:
        ...
        pre_edge_importance = [x[5] for x in data]
        ...
        ret['graph_data'] = get_graph_data(src, clss, pre_src, pre_graphs, pre_clss, pre_edge_importance, batch_size, device)
...
class GAEDataset(Dataset):
    ...
    def preprocess(self, ex):
        ...
        if(self.is_test):
            return [src, tgt, clss, src_sent_labels, graph, importance, src_for_sent, src_txt, tgt_txt]
        else:
            return [src, tgt, clss, src_sent_labels, graph, importance, src_for_sent]
