...
from gae import Summarizer
from torch.utils.data import DataLoader
from encoder import GCN, GCNII, BiEncoder, testSentEncoder
from trainer import train, test, test_ae, train_before_concat
...
def main():
    ...
    parser.add_argument("-data_path", default='../presumm_red_data/out_dir_disc_trunc')
    parser.add_argument("-model_path", default='projects/AutoEncoder/models/')
    parser.add_argument("-result_path", default='projects/AutoEncoder/results/')
    parser.add_argument("-embedding_path", default='gignore/glove/glove.840B.300d.txt')
    parser.add_argument("-vocab_path", default='gignore/vocab.txt')
    parser.add_argument("-temp_dir", default='/home/LAB/zhuhd/projects/AutoEncoder/temp')
    ...
    if args.mode == "train":
        if args.train_before_concat:
            train_before_concat(args, encoder, device, loader)
        else:
            train(args, encoder, device, loader)
    elif args.mode == "test":
        test(args, encoder, device, loader)
    elif args.mode == "test_ae":
        test_ae(args, encoder, device, loader)
