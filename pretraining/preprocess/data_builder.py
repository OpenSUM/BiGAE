...
class Tokenize:
    ...
    def __call__(self, data):
        ...
        src_result = self.corenlp.annotate(src_txt)
        ...
        src_txt = " ".join(src_txt[: self.max_src_nsents])
        return src_txt.split(" ")
        """
        if data is None:
            return data
        ...
        src_result = self.corenlp.annotate(src_txt)
        tgt_result = self.corenlp.annotate(tgt_txt)
...
class BertData():
    def __init__(self, args):
        self.args = args
        self.tokenizer = Tokenize(100) #BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
        self.vocab = Vocab(args.vocab_path, args.vocab_size)
    
    def convert_to_ids(self, tokens):
        return [self.vocab.word2id(token) for token in tokens]
    ...
    def preprocess(self, src, tgt, sent_labels, use_bert_basic_tokenizer=False, is_test=False):
        ...
        # delete sentence which is too short
        idxs = [i for i, s in enumerate(src) if (len(s) > self.args.min_src_ntokens_per_sent)] 
        _sent_labels = [0] * len(src)
        for l in sent_labels:
            _sent_labels[l] = 1
        # truncate sentence which is too long 
        src = [src[i][:self.args.max_src_ntokens_per_sent] for i in idxs]
        sent_labels = [_sent_labels[i] for i in idxs]
        src = src[:self.args.max_src_nsents]
        sent_labels = sent_labels[:self.args.max_src_nsents] # update sent labels accordingly
        
        # get merged new src and insert cls_token
        src_txt = [' '.join(sent) for sent in src]
        text = ' '.join(src_txt)
        src_len = [len(sent) for sent in src]
        
        # get src ids
        src_subtokens = [self.tokenizer(tk) for tk in src_txt]
        src_subtoken_idxs = [self.convert_to_ids(tk) for tk in src_subtokens]
        clss_id = [0]
        for i in range(len(src_subtoken_idxs)-1): 
            clss_id.append(clss_id[-1]+len(src_subtoken_idxs[i]))
        src_subtoken_idx = []
        for d in src_subtoken_idxs:
            src_subtoken_idx.extend(d)

        #print("L312", text, src_subtokens, src_subtoken_idxs)
        
        # get tgt ids
        tgt_txt = '<q>'.join([' '.join(tt) for tt in tgt])
        tgt_subtokens_str = " ".join(
            [' '.join(self.tokenizer(' '.join(tt))) for tt in tgt]) 
        tgt_subtoken = tgt_subtokens_str.split()[:self.args.max_tgt_ntokens]
        if ((not is_test) and len(tgt_subtoken) < self.args.min_tgt_ntokens):
            return None
        tgt_subtoken_idxs = self.convert_to_ids(tgt_subtoken)
        """
        # get segment ids
        ...
        for i, s in enumerate(segs):
            if (i % 2 == 0):
                ...
                segments_ids += s * [1]
        # get all sentence node ids
        cls_ids = [i for i, t in enumerate(src_subtoken_idxs) if t == self.cls_vid]
        """
        return src_subtoken_idx, sent_labels, tgt_subtoken_idxs, None, clss_id, src_txt, tgt_txt
